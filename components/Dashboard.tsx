...
type Affectation = {
  id:string; Date:string; Sultant:string; periode:string;
  mission:{ id:string; Client:string; Code:string; Color:string }|null;
  absence:{ id:string; code:string; nom:string; color:string }|null;
};
type Objectif = { sultant_id:string; annee:number; mois:number; jours:number };

const months   = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"];

// Formate un nombre de jours : 1 → "1", 1.5 → "1,5", 0 → "·"
...
export default function Dashboard() {
  ...
  useEffect(()=>{
    const load = async ()=>{
      setLoading(true);
      const [{ data:s },{ data:m },{ data:ab },{ data:a },{ data:o }] = await Promise.all([
        supabase.from("Sultant").select("*"),
        supabase.from("Mission").select("*"),
        supabase.from("Absence").select("*"),
        supabase.from("Affectation")
          .select("id,Date,Sultant,periode,mission:Mission(id,Client,Code,Color),absence:Absence(id,code,nom,color)")
          .gte("Date",`${year}-01-01`).lte("Date",`${year}-12-31`),
        supabase.from("Objectif").select("sultant_id,annee,mois,jours").eq("annee",year),
      ]);
      setConsultants(s||[]); setMissions(m||[]); setAbsences(ab||[]);
  ...
  const missionCodes = useMemo(()=>
    missions.filter((m,i,arr)=>arr.findIndex(x=>x.Code===m.Code)===i).sort((a,b)=>a.Code.localeCompare(b.Code))
  ,[missions]);
  const toggle  = <T,>(l:T[], x:T)=>l.includes(x)?l.filter(i=>i!==x):[...l,x];
  const [selMission, setSelMission] = useState<string[]>([]);

  // Seulement les affectations de missions (pas absences) pour le tableau principal
  ...
  // Toutes affectations filtrées (pour objectifs)
  const filteredAll = useMemo(()=>affectations.filter(a=>{
    if (selCon.length>0 && !selCon.includes(a.Sultant)) return false;
    return true;
  }),[affectations,selCon]);

  // Rows tableau principal
  const rows = useMemo(()=>{
    ...
    filteredMissions.forEach(a=>{
      ...
      row.byMonth.set(mi,(row.byMonth.get(mi)??0)+jours);
    });
  ...
  // Jours réalisés par mois (missions seulement)
  ...
  const monthTotals = months.map((_,mi)=>rows.reduce((s,r)=>s+(r.byMonth.get(mi)??0),0));

  // Synthèse absences
  const absenceSynth = useMemo(()=>{
    const map = new Map<string,{ label:string; color:string; byMonth:Map<number,number> }>();
    filteredAll.filter(a=>!!a.absence).forEach(a=>{
      const mi = new Date(`${a.Date}T12:00:00`).getMonth();
      const key = a.absence!.code;
      if (!map.has(key)) map.set(key,{ label:`${a.absence!.code} — ${a.absence!.nom}`, color:a.absence!.color, byMonth:new Map() });
      const row = map.get(key)!;
      const jours = a.periode==="journee" ? 1 : 0.5;
      row.byMonth.set(mi,(row.byMonth.get(mi)??0)+jours);
    });
    ...
    <div style={{ paddingTop:58 }}>
      ...
      <div style={{ padding:"1.2rem 1.5rem", maxWidth:1300, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.7rem", marginBottom:"1.2rem", flexWrap:"wrap" }}>
          <h1 style={{ margin:0, fontSize:"1.3rem", color:NAVY }}>📊 Tableau de bord {year}</h1>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"1rem", marginBottom:"1.2rem", padding:"0.8rem 1rem", background:"#f8f9fa", borderRadius:8 }}>
          <div>
            <div style={{ fontSize:"0.72rem", fontWeight:"bold", color:"#888", textTransform:"uppercase", marginBottom:"0.35rem" }}>
              Consultants {selCon.length>0&&<button onClick={()=>setSelCon([])} style={{ marginLeft:6, fontSize:"0.68rem", color:"#e74c3c", background:"none", border:"none", cursor:"pointer" }}>✕</button>}
            </div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"0.3rem" }}>
              {consultants.map(c=><button key={c.id} onClick={()=>setSelCon(p=>toggle(p,c.id))} style={pill(selCon.includes(c.id))}>{c.Nom} {c.Prénom}</button>)}
            </div>
          </div>
          <div>
            <div style={{ fontSize:"0.72rem", fontWeight:"bold", color:"#888", textTransform:"uppercase", marginBottom:"0.35rem" }}>
              Clients {selClient.length>0&&<button onClick={()=>setSelClient([])} style={{ marginLeft:6, fontSize:"0.68rem", color:"#e74c3c", background:"none", border:"none", cursor:"pointer" }}>✕</button>}
            </div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"0.3rem" }}>
              {clients.map(c=><button key={c} onClick={()=>setSelClient(p=>toggle(p,c))} style={pill(selClient.includes(c),"#8e44ad")}>{c}</button>)}
            </div>
          </div>
          <div>
            <div style={{ fontSize:"0.72rem", fontWeight:"bold", color:"#888", textTransform:"uppercase", marginBottom:"0.35rem" }}>
              Missions {selMission.length>0&&<button onClick={()=>setSelMission([])} style={{ marginLeft:6, fontSize:"0.68rem", color:"#e74c3c", background:"none", border:"none", cursor:"pointer" }}>✕</button>}
            </div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"0.3rem" }}>
              {missionCodes.map(m=>(
                <button key={m.Code} onClick={()=>setSelMission(p=>toggle(p,m.Code))}
                  style={{ padding:"0.22rem 0.65rem", border:`1.5px solid ${m.Color}`, borderRadius:20, backgroundColor:selMission.includes(m.Code)?m.Color:"white", color:selMission.includes(m.Code)?"white":m.Color, cursor:"pointer", fontSize:"0.76rem", fontWeight:500 }}>
                  {m.Code}
                </button>
              ))}
            </div>
        ...
        <div style={{ display:"flex", gap:"1rem", marginBottom:"1.2rem", flexWrap:"wrap" }}>
          <KPI value={grandReal} label="jours réalisés" color={NAVY} />
          <KPI value={grandObj>0?grandObj:"—"} label="jours prévus" color="#27ae60" />
        ...
        {/* Tableau missions */}
        {rows.length===0 ? (
          <div style={{ textAlign:"center", color:"#888", padding:"2rem" }}>Aucune donnée.</div>
        ) : (
          <div style={{ overflowX:"auto", marginBottom:"2rem" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.8rem" }}>
              <thead>
                <tr style={{ backgroundColor:NAVY, color:"white" }}>
                  <th style={{ padding:"0.5rem 0.8rem", textAlign:"left", minWidth:140 }}>
                    {groupBy==="mission"?"Mission":groupBy==="client"?"Client":"Consultant"}
                  </th>
                  {months.map((m,i)=>(
                    <th key={i} style={{ padding:"0.4rem 0.2rem", textAlign:"center", minWidth:38, fontSize:"0.72rem" }}>{m}</th>
                  ))}
                ...
                {rows.map((row,i)=>{
                  ...
                  return (
                    <tr key={row.key} style={{ backgroundColor:i%2===0?"#f9f9f9":"white" }}>
                      ...
                      {months.map((_,mi)=>{ const v=row.byMonth.get(mi)??0; return <td key={mi} style={{ padding:"0.4rem 0.2rem", textAlign:"center", color:v===0?"#ddd":"#000", fontWeight:v>0?"600":"400", fontSize:"0.78rem" }}>{fj(v)}</td>; })}
              ...
              <tfoot>
                ...
                <tr style={{ backgroundColor:"#27ae60", color:"white", fontWeight:"bold" }}>
                  ...
                  {objByMonth.map((v,i)=><td key={i} style={{ padding:"0.4rem 0.2rem", textAlign:"center", fontSize:"0.78rem" }}>{v>0?v:"·"}</td>)}
                  <td style={{ padding:"0.4rem 0.5rem", textAlign:"center", borderLeft:"2px solid rgba(255,255,255,0.2)" }}>{grandObj>0?grandObj:"—"}</td>
                </tr>
                {/* Ligne écart */}
                {grandObj>0 && (
                  <tr style={{ backgroundColor:NAVY, color:"white", fontWeight:"bold" }}>
                    ...
                    {realByMonth.map((v,i)=>{
                      const e=v-(objByMonth[i]??0);
                      const hasObj=(objByMonth[i]??0)>0;
                      return <td key={i} style={{ padding:"0.4rem 0.2rem", textAlign:"center", fontSize:"0.75rem", color:hasObj?(e>=0?"#7dcea0":"#f1948a"):"rgba(255,255,255,0.3)" }}>{hasObj?(e>=0?`+${fj(e)}`:fj(e)):"·"}</td>;
                    })}
                    <td style={{ padding:"0.4rem 0.5rem", textAlign:"center", borderLeft:"2px solid rgba(255,255,255,0.2)", color:(grandReal-grandObj)>=0?"#7dcea0":"#f1948a" }}>
                      {(grandReal-grandObj)>=0?`+${fj(grandReal-grandObj)}`:fj(grandReal-grandObj)}
                    </td>
                  </tr>
                )}
              </tfoot>
        ...
        {absenceSynth.length>0 && (
          <div>
            ...
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.8rem" }}>
                <thead>
                  <tr style={{ backgroundColor:"#7f8c8d", color:"white" }}>
                    <th style={{ padding:"0.4rem 0.8rem", textAlign:"left", minWidth:140 }}>Motif</th>
                    {months.map((m,i)=><th key={i} style={{ padding:"0.3rem 0.2rem", textAlign:"center", minWidth:38, fontSize:"0.72rem" }}>{m}</th>)}
                    <th style={{ padding:"0.3rem 0.5rem", textAlign:"center", minWidth:50, borderLeft:"2px solid rgba(255,255,255,0.2)" }}>Total</th>
                  </tr>
                </thead>
                <tbody>
                  {absenceSynth.map((row,i)=>{
                    ...
                    return (
                      <tr key={row.key} style={{ backgroundColor:i%2===0?"#f9f9f9":"white" }}>
                        ...
                        {months.map((_,mi)=>{ const v=row.byMonth.get(mi)??0; return <td key={mi} style={{ padding:"0.3rem 0.2rem", textAlign:"center", color:v===0?"#ddd":"#000", fontWeight:v>0?"600":"400", fontSize:"0.78rem" }}>{fj(v)}</td>; })}
                        <td style={{ padding:"0.3rem 0.5rem", textAlign:"center", fontWeight:"bold", borderLeft:"2px solid #eee", backgroundColor:`${row.color}18` }}>{fj(total)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                <tfoot>
                  <tr style={{ backgroundColor:"#7f8c8d", color:"white", fontWeight:"bold" }}>
                    <td style={{ padding:"0.4rem 0.8rem" }}>TOTAL</td>
                    {months.map((_,mi)=>{
                      const t=absenceSynth.reduce((s,r)=>s+(r.byMonth.get(mi)??0),0);
                      return <td key={mi} style={{ padding:"0.3rem 0.2rem", textAlign:"center", fontSize:"0.78rem" }}>{fj(t)}</td>;
                    })}
                    <td style={{ padding:"0.3rem 0.5rem", textAlign:"center", borderLeft:"2px solid rgba(255,255,255,0.2)" }}>
                      {fj(absenceSynth.reduce((s,r)=>s+Array.from(r.byMonth.values()).reduce((a,b)=>a+b,0),0))}
                    </td>
                  </tr>
                </tfoot>
              </table>
